...
- Every new non-system message has exactly one sender: `user_id` or `bot_id`.
  Existing sender-less tombstones created by profile deletion remain readable
  history and must never be reassigned to a bot.
...
- Adds `messages.bot_id uuid null references public.bots(id)` and enforces the
  one-sender rule for new writes while preserving legacy human tombstones.
...
- [x] **Step 3: Add public bot identity and ownership tables**
...
- [x] **Step 4: Add private token and delivery tables**
...
- [x] **Step 5: Add the bot sender column without changing human rows**

```sql
alter table public.messages add column if not exists bot_id uuid null references public.bots(id) on delete restrict;

alter table public.messages drop constraint if exists messages_sender_shape_check;
alter table public.messages add constraint messages_sender_shape_check check (
...
Add a `BEFORE INSERT` trigger that requires exactly one sender for every new
non-system message and no sender for system messages. Preserve existing
sender-less tombstones produced by human profile deletion, record their
aggregate baseline before rollout, and never invent or backfill a bot sender
for historical rows. Bot deletion remains archive-only while referenced
messages exist.

- [x] **Step 6: Add service-role-only gateway RPCs**
...
- [x] **Step 7: Adapt message notifications transactionally**
...
- [x] **Step 8: Add database smoke coverage**
...
- [x] **Step 9: Run schema checks and commit**
...
- [x] **Step 1: Add Zod as a direct API dependency**
...
- [x] **Step 3: Implement token generation and constant-time verification**
...
- [x] **Step 4: Write and implement Zod method schemas**
...
- [x] **Step 5: Implement the repository token lookup**
...
- [x] **Step 6: Implement safe API errors**
...
- [x] **Step 1: Write a failing method-router test**
...
- [x] **Step 2: Implement the isolated Express app**
...
- [x] **Step 3: Implement identity and command methods**
...
- [x] **Step 4: Implement message methods through one transactional RPC**
...
- [x] **Step 5: Implement bounded file access**
...
- [x] **Step 6: Add the dedicated build/start entry**
...
- [x] **Step 7: Run gateway tests and commit**
...
- [x] **Step 1: Write failing SSRF tests**
...
- [x] **Step 2: Implement URL and DNS validation**
...
- [x] **Step 3: Write failing update-delivery tests**
...
- [x] **Step 4: Implement polling and webhook mutual exclusion**
...
- [x] **Step 5: Implement webhook retry processing**
...
- [x] **Step 6: Add retention cleanup**
...
- [x] **Step 7: Run security and delivery tests, then commit**
...
- [x] **Step 1: Write failing management-auth tests**
...
- [x] **Step 2: Implement management authentication**
...
- [x] **Step 3: Implement bot CRUD and token lifecycle routes**
...
- [x] **Step 4: Write the frontend management client**
...
- [x] **Step 6: Add route/navigation and Playwright coverage**
...
- [x] **Step 7: Run frontend/API validation and commit**
...
- [x] **Step 2: Add bot-aware database and application types**
...
- [x] **Step 3: Update every message join consistently**
...
- [x] **Step 4: Render bot identity and deleted state**
...
- [x] **Step 5: Add bot search grouping**
...
- [x] **Step 6: Adapt notification presentation**
...
- [x] **Step 7: Run chat regression and commit**
...
- [x] **Step 1: Write failing deploy contract tests**
...
- [x] **Step 2: Add the dedicated runtime image**
