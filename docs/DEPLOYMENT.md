...
- `VITE_AUTH_CAPTCHA_PROVIDER`, `VITE_AUTH_CAPTCHA_SITE_KEY`, and optional `VITE_AUTH_GATEWAY_URL` are public build-time values for signup, confirmation resend, and recovery bot protection. All three operations use `auth-yandex-gateway`; missing or unsupported CAPTCHA configuration fails closed. Configure the matching `KUB_AUTH_CAPTCHA_PROVIDER` and Yandex or Turnstile secret only in the Edge Function runtime environment.
