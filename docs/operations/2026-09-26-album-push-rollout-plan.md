...
Owner: shared backend (Codex). Stage: design reviewed, no schema or runtime
change applied. The separate iOS/MacOS task owns iPhone PWA behavior.
...
- The live `_enqueue_push_after_notification_insert()` trigger creates Web
  ...
  branch currently selects Android FCM only. Check the separate WNS path before
  promising album behavior on Windows.
...
- PostgREST RPC/grant contract and mock Web Push/FCM/WNS dispatch: one logical
  delivery per eligible target, neutral OS content, exact tap route. Inspect
  the live WNS path before treating its coverage as complete.
