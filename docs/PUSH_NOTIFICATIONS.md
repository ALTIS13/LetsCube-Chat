...
- Web Push eligibility is checked twice: `push_outbox_claim` selects an unread row, and `push_outbox_delivery_recheck` locks and rechecks that exact claim immediately before the provider call. A row read after the first claim is suppressed as `read`; an inactive subscription is suppressed; an active foreground session releases the claim without suppressing the unread row so delivery can resume after the user leaves. If the recheck RPC is unavailable or returns an unknown state, the dispatcher fails closed and does not give the payload to the provider.
...
- Production contained one active Apple Web Push subscription whose `last_seen_at` had not advanced since 2026-06-22. Five deliveries from 2026-07-01 exhausted retries with HTTP 403 and two were accepted after retries.
- A live probe found the root cause: Supabase Cron/`send-push-notifications` and the legacy API worker were consuming the same Web Push outbox. The five-second legacy loop used a different VAPID subject, received Apple's sanitized `BadJwtToken`, and could exhaust the retry limit before the canonical minute cron delivered the row.
- Supabase Cron/`send-push-notifications` is now the sole production push dispatcher. The API worker's legacy dispatcher is disabled by default and requires the explicit local opt-in `PUSH_DISPATCHER_ENABLED=1`; media variant processing remains independent.
- The configured public VAPID value matches the web production bundle and the configured private/public VAPID pair is cryptographically valid. Dispatchers retain only a sanitized provider reason and never log the subscription endpoint.
- After the single-owner worker deployment, a second live Apple probe was accepted with `attempt_count=0` and no error. Both probe notifications and their cascading outbox rows were removed after verification.
