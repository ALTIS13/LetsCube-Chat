...
- `0.2.6/10` uses exact-origin Rust commands for real native visibility and Windows toast delivery. The native command validates bounded title/body, a known message/task/system group and a relative production-app route, XML-escapes untrusted preview text, sets a stable eight-character tag plus semantic group and emits only a safe internal action event.
- A direct WinRT physical probe registered handler `ru.letscube.messenger` and stored one `toast` row with the expected `codexqa1/system` tag/group without reading or printing its payload. Frontend and Tauri contract tests passed 23/23, Rust passed 28/28, and the full native lifecycle matrix passed baseline 3/3 plus all five startup/update scenarios 1/1. The signed 2,297,595-byte installer has SHA-256 `5fee9bf5250b1bc7ab7dc232419789e3d40a07b86e749836e1972f5957ab7af9`.
- Stable remains `0.2.3/7`. Production frontend deployment and the physical hidden-window same-chat replacement/task/action/reconnect pass remain release gates; killed-process delivery remains a separate WNS/device-token/backend stage.
