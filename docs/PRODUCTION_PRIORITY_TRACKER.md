...
Status: `[~]` p1sms delivery is enabled for authenticated accounts after physical pilot QA. The current route is digital SMS first with a message-scoped Telegram fallback only after `not_delivered`; mandatory phone enforcement remains a separate future decision.
...
- `[x]` p1sms source foundation: strict 46-character moderated template, hard 65-character guard, one immediate `digit` message per request, message-scoped `not_delivered -> telegram_auth` fallback, `letscube-otp` tag, redirect blocking, Standard Webhooks verification, deployed GoTrue digits-only `sms.phone` normalization with guarded legacy fallback, HMAC claim checks, concurrent-retry idempotency, per-user/per-phone server ceilings and safe result categories. LETSCUBE neither polls delivery nor issues a second provider request. Automated validation makes no real delivery request.
...
- `[~]` Use the dedicated `auth.resend({ type: "phone_change" })` path for resend, preserve the 120-second UI/server cooldown and server-side per-user/per-phone limits. Broader CAPTCHA/cost alerting remains a global-rollout gate.
