...
9. `[~]` Complete the external Windows release gates. The `0.2.4` candidate moves tray-process notifications to the global Notification Center realtime stream, groups messages by chat, keeps tasks separate, reconciles after reconnect and restores the hidden main window on a safe notification action. Remaining external work is Authenticode/SmartScreen reputation, a broader Windows 10/11 device matrix and true killed-process delivery through a separate WNS/device-token/backend design. These do not reopen the completed signed-updater foundation.
