...
- `[~]` Native push release QA: real owner-to-client message delivery, sender exclusion, category preference suppression, same-chat collapse, chat read-sync/tap routing, separate task delivery, restart registration recovery and Android 16 Google Play emulator coverage pass. Remaining: killed-app delivery, staff-role task-detail routing and a broader physical-device matrix.
