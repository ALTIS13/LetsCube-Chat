...
authenticated signed-release FCM, long-session and wider device-matrix
QA remain open. The owner confirmed signed `0.1.8/9` push arrival on a
physical official-GMS device on 2026-09-24; latency and tap routing on that
handset were not measured. [Release evidence](operations/2026-09-23-android-0.1.8-release.md).
...
and guest cold/resume 1/1 without modifying the original AVD. A follow-up on
the same read-only AVD passed live background and normally killed FCM cards;
...
46. `[~]` Tester's report, 2026-09-20, all of it from a phone. Eight items,
    ...
    **h. A forwarded message does not say where it came from.** «плюс не вижу
    ...
    ### The owner's decisions, 2026-09-20, after the triage

    **(g) was (f) all along.** He explained what «поставить "+"» meant: it is
    about the message that went out *without* its caption — «ответить сам на
    своё сообщение он не может, чтобы хотя бы так дать описание какое-то или
    пометку на что обратить внимание собеседнику». So the tester was not
    asking for a reaction or a pin. He was trying to repair the defect in (d)
    by hand, and the product would not let him. Two complaints, one cause.

    **Reply and forward become a swipe, left and right, as in Telegram** —
    «это по аналогии с телеграммом сделай действиями влево/вправо по
    сообщению, также эти функции есть в ПКМ». His screenshot of our own
    long-press menu shows the full set already present: Ответить, Закрепить,
    Копировать текст, Переслать, Удалить, Выделить.

    So the gesture question the triage left open is settled, and settled
    **without** overturning D-071: the long-press menu keeps everything it has,
    and the swipe is added beside it. That also answers the tap problem on a
    photo — a horizontal swipe does not compete with the tap that opens the
    viewer, where a long press does.

    Two things the swipe has to get right, both of which are why it is worth
    measuring Telegram rather than inventing:

    - **Which direction is which.** Telegram assigns one direction to reply and
      the other to forward; do not choose by taste.
    - **It must not fight the horizontal swipe (b) is about.** The photo viewer
      needs left/right to move between images. A swipe on a *message row* and a
      swipe inside an *open viewer* are different surfaces, but the boundary
      has to be deliberate rather than discovered.

    **A device is available.** The owner has connected an Android phone over
    ADB with debugging on, carrying both Telegram and `com.kub.messenger`, and
    authorised going anywhere in his personal account — «но не пали данные если
    какие-то чувствительные будут». That unblocks (a), which the triage
    correctly refused to guess at.

    **The rule for using it, which is not optional.** Access is authorised;
    exposure is not. Measure what is being measured and nothing else: never
    transcribe, screenshot into a kept file, report or commit any conversation
    content, contact name, phone number or personal media. Telegram's own
    appearance settings carry a **synthetic** message preview and a size
    slider — that is the surface to measure on, and it contains nobody's data.
...
Status: `[~]` active. Android `0.1.9/10` is signed and published in Stable;
...
- `[~]` 2026-09-23 supersedes the old Realme check-in note above: the stale microG account binding was removed on this designated test handset, so check-in, FCM token, background/killed-process card and exact-route QA succeeded. The `0.1.8/9` debug candidate adds non-collapsible data-only chat push for new Android versions while old devices retain their existing payload. Two QA chats queued offline produced two distinct cards after reconnect on Realme. Google Play API 33 and API 34 AVDs registered FCM devices, received live background and `am kill` chat cards, and actual System UI card taps reached exact messages. API 33 additionally preserved another chat card after a tap. Signed APK verification and Stable publication are now complete as recorded above. On 2026-09-24 the owner confirmed push arrival on a physical official-GMS handset running signed `0.1.8/9` in another location; latency and the wider long-session/device matrix remain unmeasured there. In-app notifications still own state.
- `[~]` 2026-09-24 push/message UX follow-up: the native outbox now has token-bound atomic claims and the production generic push cron runs every 10 seconds instead of every minute, after a verified database backup and observed healthy dispatch. Receipt colour, message-size scale, sound and guarded cached-history reads are included in signed Android 0.1.9/build 10 Stable and the live web. Existing Android notification-channel sound choices are preserved. Signed authenticated FCM, physical official-GMS latency/audio and long-session QA remain open. See `docs/operations/2026-09-24-push-and-message-ux.md` and `docs/operations/2026-09-24-android-0.1.9-release.md`.
