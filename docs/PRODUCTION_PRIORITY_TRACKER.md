...
- `[~]` SMS provider rollout is the next backend/auth stage after the current audit. The p1sms implementation is currently limited to a provider-disabled server foundation: no real SMS, Auth hook activation, cutoff or RLS enforcement before provider approval and physical QA.
...
- `[x]` Provider-disabled source foundation updated for p1sms: strict 46-character template, hard 65-character guard, one immediate `digit` message per request, `letscube-otp` tag, redirect blocking, Standard Webhooks verification, `new_phone` routing, HMAC claim checks, concurrent-retry idempotency, per-user/per-phone server ceilings and safe result categories. It is not deployed or enabled and has made no real SMS request.
- `[ ]` Provider activation: add only `P1SMS_API_KEY` and hook/HMAC secrets to trusted server storage. Keep the shared LETSCUBE account isolated by never calling p1sms account, sender, history, scheduling, reject, phone-base or blacklist APIs.
