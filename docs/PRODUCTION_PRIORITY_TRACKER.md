...
9. `[~]` Complete the external Windows release gates. The signed `0.2.4` build `8` candidate is published to the opt-in Test updater channel only. It moves tray-process notifications to the global Notification Center realtime stream, groups messages by chat, keeps tasks separate, reconciles after reconnect and restores the hidden main window on a safe notification action. Stable promotion is gated on a physical hidden-window message/task/action and reconnect pass. Remaining external work is Authenticode/SmartScreen reputation, a broader Windows 10/11 device matrix and true killed-process delivery through a separate WNS/device-token/backend design. These do not reopen the completed signed-updater foundation.

## Last Confirmed Deploy Baseline

- Production web baseline: `0bbfca210ebf6afbfc61ee17005c5fa9aedc5e78` (global Windows tray notification reconciliation and safe action routing). The stable startup geometry, transient update confirmation and cryptographically verified publisher remain included from their earlier baselines.
- Coolify app: `letscube-web`.
- Public app: `https://app.letscube.ru`.
- Auto deploy: GitHub webhook to Coolify is active for `letscube-web`; deployment `d10dqxllayooif96npfbl4dq` completed exact commit `0bbfca210ebf6afbfc61ee17005c5fa9aedc5e78`, passed its first healthcheck and completed the rolling update. The chat-summary and access-snapshot RPC build flags remain enabled.
...
- `[~]` Windows public release gate: repeatable isolated Tauri/WebView2 QA, same-version repair, silent uninstall, clean reinstall and real signed cross-version updater application pass. The `0.2.4/8` tray-notification candidate remains in Test pending a physical hidden-window message/task/action pass; Authenticode/SmartScreen and killed-process notifications remain open.
