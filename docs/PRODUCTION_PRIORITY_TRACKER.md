...
7. `[~]` Native/mobile packaging resumed: Windows Tauri secure startup and signed updater are complete; Android now has an unpublished signed `0.1.2/3` candidate with authenticated upgrade plus background/killed FCM card/tap/read-sync acceptance. Independent foreground FCM transport, physical media/geolocation/history/footer, domain verification and publication remain active gates.
...
7. `[~]` Complete the Capacitor Android release candidate. The unpublished `0.1.2/3` APK/AAB is signed and verified; baseline-to-final same-key package-data plus authenticated session/chat/native-registration retention, explicit warm/cold/killed callbacks, malformed/foreign rejection, Android 13/14/16 Google Play emulators and the Android 15 official-GMS Nothing lifecycle matrix passed. Signed-final background/killed system-card, exact-chat tap and read-sync acceptance also passed. Independent foreground FCM transport, physical media/geolocation/large-chat/footer QA, production App Link verification, external off-device backup, Play setup and catalog publication remain external gates. The currently published `0.1.0` catalog APK remains internal/debug.
