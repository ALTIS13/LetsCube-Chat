...
- `[x]` p1sms source foundation: strict 44-character moderated template, hard 65-character guard, one immediate `digit` message per request, message-scoped `agg_error -> telegram_auth`, `not_delivered -> telegram_auth` and `error -> telegram_auth` fallbacks, no undocumented request fields, redirect blocking, four-digit HMAC-only storage, 10-minute TTL, five verification attempts, per-user/per-phone server ceilings and safe result categories. P1SMS support confirmed that `agg_error` is a separate not-sent status. LETSCUBE neither polls delivery nor issues a second provider request. Automated validation makes no real delivery request.
