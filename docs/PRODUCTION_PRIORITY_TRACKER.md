...
7. `[~]` Native/mobile packaging resumed: Windows Tauri secure startup and signed updater are complete; Android now has an unpublished signed `0.1.2/3` candidate with authenticated upgrade, foreground/background/killed FCM acceptance, exact taps/read-sync, offline/reconnect, first-unread anchoring and bounded geolocation on official-GMS Nothing. Physical media/capture and remaining history/footer/logout cases, domain verification and publication remain active gates.
...
7. `[~]` Complete the Capacitor Android release candidate. The unpublished `0.1.2/3` APK/AAB is signed and verified. Android 13/14/16 Google Play emulators passed the emulator lifecycle matrix. Separately, official-GMS Nothing passed same-key package-data and authenticated session/chat/native-registration retention, warm/cold/killed callbacks, malformed/foreign rejection, signed-final foreground/background/killed FCM, exact-chat taps/read-sync, authenticated offline/reconnect, first-unread anchoring and bounded geolocation. Physical media/camera/regular-video/video-circle/voice, no-unread bottom, fast-upward scroll, prepend anchor, footer stability and explicit logout/login remain mandatory QA gates; Task 4 stays open. Production App Link verification, external off-device backup, Play setup and catalog publication also remain external gates. The currently published `0.1.0` catalog APK remains internal/debug.
