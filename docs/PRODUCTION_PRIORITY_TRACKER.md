...
- `[~]` SMS provider rollout is the next backend/auth stage after the current audit. The accepted SMS.RU implementation is currently limited to a provider-disabled server foundation: no real SMS, Auth hook activation, cutoff or RLS enforcement before provider approval.
...
- `[x]` Delivery architecture selected: Supabase Auth HTTP Send SMS Hook -> server-side LETSCUBE adapter -> SMS.RU. Supabase Auth remains responsible for OTP generation and verification.
- `[~]` Provider-disabled source foundation added: strict 46-character template, hard 65-character guard, POST-form SMS.RU adapter, Standard Webhooks verification, HMAC claim checks, concurrent-retry idempotency, per-user/per-phone server ceilings and safe result categories. It is not deployed or enabled and has made no real SMS request.
- `[ ]` Provider onboarding: approve production terms and sender `LETSCUBE`, then add only `SMS_RU_API_ID` and hook/HMAC secrets to trusted server storage. Runtime does not use the SMS.RU login/password pair.
- `[ ]` Review, rehearse and manually apply `.migration-backup/supabase/migrations/20260810_smsru_phone_verification_foundation.sql` after a current backup. Its policy, delivery and enforcement defaults are off.
