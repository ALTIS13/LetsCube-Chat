...
- `[~]` 2026-09-24 push/message UX follow-up: the native outbox now has token-bound atomic claims and the production generic push cron runs every 10 seconds instead of every minute, after a verified database backup and observed healthy dispatch. Receipt colour, message-size scale and sound improvements are source-only until a new Android build; existing Android notification-channel sound choices are preserved. See `docs/operations/2026-09-24-push-and-message-ux.md` for evidence, rollback and remaining physical QA.
