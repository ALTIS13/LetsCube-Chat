...
Status: `[~]` p1sms delivery is restricted to administrators after physical pilot QA. The current route is digital SMS first with message-scoped Telegram fallback branches after `not_delivered` or a terminal provider error; mandatory phone enforcement remains a separate future decision.
