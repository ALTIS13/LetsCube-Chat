...
8. `[x]` Replace the retired Electron spike with a clean-profile Tauri 2 Windows client. The one-window secure startup, tray/single-instance behavior, Stable/Test channels and signed Tauri updater are complete. Physical `0.2.0 -> 0.2.1` trust-chain and `0.2.1 -> 0.2.2` production-update rehearsals passed. Stable download/updater catalogs publish `0.2.2` build `6` from the same immutable artifact. A successful version change shows a compact four-second confirmation and then frees the top-right area for future call controls.
...
- Production web baseline: `1f389e0baf456b9af48618fa19ec8d0a746adb34` (stable Windows startup handoff plus transient update confirmation). The cryptographically verified publisher baseline remains included from `3fcea10c0d6147e60ec55aaafac9b655a39fea0d`.
- Coolify app: `letscube-web`.
- Public app: `https://app.letscube.ru`.
- Auto deploy: GitHub webhook to Coolify is active for `letscube-web`; deployment `e3h1h6pl55f77mri39jfjv4m` completed exact commit `1f389e0baf456b9af48618fa19ec8d0a746adb34`, passed its first healthcheck and completed the rolling update. The chat-summary and access-snapshot RPC build flags remain enabled.
...
- The Windows stable download catalog now offers LETSCUBE `0.2.2` build `6`. The Tauri Stable and Test updater manifests reference the same immutable 2,327,965-byte installer with SHA-256 `a20de9643869f99d0e2638815c6eec4ab41b176021692732c5fd5f8df90fc75e`; the installer is updater-signed and physically accepted by Tauri. The production handoff keeps one fixed scene across navigation, remains readable for at least 2.2 seconds and holds the confirmed state for at least 0.9 seconds before fading.
