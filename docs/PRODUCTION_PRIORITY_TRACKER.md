...
9. `[~]` Complete the external Windows release gates. Physical `0.2.4/8` QA proved that the tray process and Realtime stay alive, but also exposed two Windows-specific gaps: WebView2 remains DOM-visible after the native window is hidden, and the desktop backend of the Tauri notification plugin discards `id/group/extra` while swallowing the asynchronous toast error. Signed `0.2.6/10` replaces that path with exact-origin `desktop_is_main_visible` and `desktop_notify` commands, native Windows `tag/group`, bounded content and a safe relative action route. It is awaiting production frontend deployment and a physical hidden-window message/task/action/reconnect pass before replacing the Test candidate; Stable remains `0.2.3/7`. Remaining external work is Authenticode/SmartScreen reputation, a broader Windows 10/11 device matrix and true killed-process delivery through a separate WNS/device-token/backend design.

## Last Confirmed Deploy Baseline

- Production web baseline: `c078d6b1d2f7bef722b8d3ed946d2f9c9bbb1ad8` (production-safe Notification Center mutation QA over the global Windows tray notification reconciliation baseline). The stable startup geometry, transient update confirmation and cryptographically verified publisher remain included from their earlier baselines.
- Coolify app: `letscube-web`.
- Public app: `https://app.letscube.ru`.
- Auto deploy: GitHub webhook to Coolify is active for `letscube-web`; deployment `u92si2xe9hv7r42mqhydargu` completed exact commit `c078d6b1d2f7bef722b8d3ed946d2f9c9bbb1ad8`, passed its first healthcheck and completed the rolling update. The chat-summary and access-snapshot RPC build flags remain enabled.
...
- `[~]` Windows public release gate: repeatable isolated Tauri/WebView2 QA, same-version repair, silent uninstall, clean reinstall and real signed cross-version updater application pass. Physical `0.2.4/8` tray QA exposed a hidden-window visibility/plugin delivery defect. Signed `0.2.6/10` contains the native Windows toast replacement and remains outside Stable pending production frontend deployment plus a physical hidden-window message/task/action pass; Authenticode/SmartScreen and killed-process notifications remain open.
