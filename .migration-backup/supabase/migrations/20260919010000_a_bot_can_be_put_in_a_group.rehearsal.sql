...
\echo == an admin, a member, a group and a private chat ==
...
insert into public.profiles (id, full_name) values ('aaaaaaaa-0000-4000-8000-00000000000a','Админ'),('bbbbbbbb-0000-4000-8000-00000000000b','Участник');
...
insert into public.chat_members (chat_id,user_id,role) values
 ...
 ('eeeeeeee-0000-4000-8000-00000000000e','bbbbbbbb-0000-4000-8000-00000000000b','member')
on conflict (chat_id,user_id) do update set role = excluded.role;
insert into public.bots (id, username, display_name, state) values
 ('bbbb0001-0000-4000-8000-000000000001','testbot_one','Бот один','active'),
 ('bbbb0002-0000-4000-8000-000000000002','testbot_two','Бот два','paused');

create or replace function pg_temp.as_user(p_who uuid) returns void language plpgsql as $f$
begin perform set_config('request.jwt.claims', json_build_object('sub',p_who,'role','authenticated')::text, true); end $f$;

\echo -- the admin sees the active bot offered and not the paused one --
select pg_temp.as_user('aaaaaaaa-0000-4000-8000-00000000000a');
set local role authenticated;
select username from public.chat_bots_available('eeeeeeee-0000-4000-8000-00000000000e');
reset role;

\echo -- a plain member sees nothing at all, rather than an error --
set local role authenticated;
select pg_temp.as_user('bbbbbbbb-0000-4000-8000-00000000000b');
select count(*) as member_sees from public.chat_bots_available('eeeeeeee-0000-4000-8000-00000000000e');
do $p$ begin
  perform public.chat_bot_add('eeeeeeee-0000-4000-8000-00000000000e','bbbb0001-0000-4000-8000-000000000001');
  raise notice 'UNEXPECTED: a plain member added a bot';
exception when others then raise notice 'a plain member is refused: %', sqlerrm; end $p$;
reset role;

\echo -- the admin adds it, restricted, and adding twice changes nothing --
set local role authenticated;
select pg_temp.as_user('aaaaaaaa-0000-4000-8000-00000000000a');
select public.chat_bot_add('eeeeeeee-0000-4000-8000-00000000000e','bbbb0001-0000-4000-8000-000000000001') as added;
select public.chat_bot_add('eeeeeeee-0000-4000-8000-00000000000e','bbbb0001-0000-4000-8000-000000000001') as added_again;
reset role;
select bot_id, privacy_mode, removed_at is null as present from public.chat_bot_members where chat_id='eeeeeeee-0000-4000-8000-00000000000e';
\echo -- and the authoriser now lets it speak, but not read everything --
select public.bot_membership_authorize_internal('bbbb0001-0000-4000-8000-000000000001','eeeeeeee-0000-4000-8000-00000000000e','send_message')->>'allowed' as may_send,
       public.bot_membership_authorize_internal('bbbb0001-0000-4000-8000-000000000001','eeeeeeee-0000-4000-8000-00000000000e','receive_all')->>'allowed' as may_read_all;

\echo -- a paused bot is refused, and a private chat is refused --
set local role authenticated;
select pg_temp.as_user('aaaaaaaa-0000-4000-8000-00000000000a');
do $p$ begin perform public.chat_bot_add('eeeeeeee-0000-4000-8000-00000000000e','bbbb0002-0000-4000-8000-000000000002');
  raise notice 'UNEXPECTED: a paused bot was added';
exception when others then raise notice 'paused bot refused: %', sqlerrm; end $p$;
do $p$ begin perform public.chat_bot_add('cccccccc-0000-4000-8000-00000000000c','bbbb0001-0000-4000-8000-000000000001');
  raise notice 'UNEXPECTED: a private chat accepted a bot this way';
exception when others then raise notice 'private chat refused: %', sqlerrm; end $p$;

\echo -- removal is soft, and re-adding resets the trust --
select public.chat_bot_remove('eeeeeeee-0000-4000-8000-00000000000e','bbbb0001-0000-4000-8000-000000000001') as removed;
reset role;
update public.chat_bot_members set privacy_mode='full', full_visibility_requested_at=now(),
       full_visibility_approved_by='aaaaaaaa-0000-4000-8000-00000000000a'
 where chat_id='eeeeeeee-0000-4000-8000-00000000000e';
select privacy_mode, removed_at is not null as gone from public.chat_bot_members where chat_id='eeeeeeee-0000-4000-8000-00000000000e';
set local role authenticated;
select pg_temp.as_user('aaaaaaaa-0000-4000-8000-00000000000a');
select public.chat_bot_add('eeeeeeee-0000-4000-8000-00000000000e','bbbb0001-0000-4000-8000-000000000001') as re_added;
reset role;
select privacy_mode as after_readd, removed_at is null as present_again,
       full_visibility_approved_by is null as trust_reset
  from public.chat_bot_members where chat_id='eeeeeeee-0000-4000-8000-00000000000e';
rollback;
