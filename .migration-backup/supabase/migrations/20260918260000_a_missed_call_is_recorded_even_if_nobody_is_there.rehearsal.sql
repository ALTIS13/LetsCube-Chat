...
\echo == a ring nobody was there for ==
...
select u.id,'00000000-0000-0000-0000-000000000000','authenticated','authenticated',u.id||'@c.invalid','',now(),now(),'{}'::jsonb,'{}'::jsonb
  from (values ('aaaaaaaa-0000-4000-8000-00000000000a'::uuid),('bbbbbbbb-0000-4000-8000-00000000000b'::uuid)) as u(id);
insert into public.profiles (id, full_name) values ('aaaaaaaa-0000-4000-8000-00000000000a','С А'),('bbbbbbbb-0000-4000-8000-00000000000b','С Б');
...
insert into public.voice_channels (id, chat_id, name, max_participants)
values ('33333333-0000-4000-8000-000000000033','cccccccc-0000-4000-8000-00000000000c','Звонок',2),
       ('44444444-0000-4000-8000-000000000044','eeeeeeee-0000-4000-8000-00000000000e','Общий',10);

\echo -- ringing for 50s: expired, but inside the grace, so the sweep leaves it --
update public.voice_channels set ring_started_at = now() - interval '50 seconds',
       ring_caller='aaaaaaaa-0000-4000-8000-00000000000a' where id='33333333-0000-4000-8000-000000000033';
select public.voice_rings_sweep_expired() as wrote_inside_the_grace;
select ring_started_at is not null as ring_still_there from public.voice_channels where id='33333333-0000-4000-8000-000000000033';

\echo -- ringing for 80s: past the grace, so it is written down --
update public.voice_channels set ring_started_at = now() - interval '80 seconds' where id='33333333-0000-4000-8000-000000000033';
select public.voice_rings_sweep_expired() as wrote_past_the_grace;
select content, system_payload->>'outcome' as outcome,
       ...
       system_payload->>'duration_ms' as duration
  from public.messages where chat_id='cccccccc-0000-4000-8000-00000000000c';
select ring_started_at is null as ring_cleared from public.voice_channels where id='33333333-0000-4000-8000-000000000033';

\echo -- running it again writes nothing: exactly one record per call --
select public.voice_rings_sweep_expired() as second_run;
select count(*) as records from public.messages where chat_id='cccccccc-0000-4000-8000-00000000000c';

\echo -- an answered ring is not the sweep's business --
update public.voice_channels set ring_started_at = now() - interval '600 seconds',
       ring_caller='aaaaaaaa-0000-4000-8000-00000000000a',
       ring_answered_at = now() - interval '590 seconds' where id='33333333-0000-4000-8000-000000000033';
select public.voice_rings_sweep_expired() as wrote_for_an_answered_ring;
select ring_answered_at is not null as answered_ring_untouched from public.voice_channels where id='33333333-0000-4000-8000-000000000033';

\echo -- and a group room is excluded, even if somebody puts a ring on it --
update public.voice_channels set ring_started_at = now() - interval '600 seconds',
       ring_caller='aaaaaaaa-0000-4000-8000-00000000000a', ring_answered_at = null
 where id='44444444-0000-4000-8000-000000000044';
select public.voice_rings_sweep_expired() as wrote_for_a_group;
select count(*) as group_messages from public.messages where chat_id='eeeeeeee-0000-4000-8000-00000000000e';

\echo == the schedule ==
select jobname, schedule, active from cron.job where jobname='letscube-voice-missed-call-sweep';
rollback;
\echo == rolled back: the job is gone again ==
select count(*) as job_rows from cron.job where jobname='letscube-voice-missed-call-sweep';
