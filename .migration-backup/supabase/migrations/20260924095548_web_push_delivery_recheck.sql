...
-- drop function if exists public.push_outbox_delivery_recheck(uuid, uuid);
begin;
...
begin
  ...
  select o.user_id, n.read_at, s.is_active
  into v_user_id, v_read_at, v_subscription_active
...
begin
  if not has_function_privilege(
      'service_role',
      'public.push_outbox_delivery_recheck(uuid,uuid)',
      'EXECUTE'
    )
  ...
  then
    raise exception 'web_push_delivery_recheck_self_check_failed';
  end if;
end
$$;
