...
\echo == two people, three sessions, and only your own are yours ==
...
select u.id,'00000000-0000-0000-0000-000000000000','authenticated','authenticated',u.id||'@f.invalid','',now(),now(),'{}'::jsonb,'{}'::jsonb
...
insert into auth.sessions (id, user_id, created_at, updated_at, refreshed_at, user_agent, ip) values
 ('11111111-0000-4000-8000-000000000001','aaaaaaaa-0000-4000-8000-00000000000a', now()-interval '5 days', now(), (now()-interval '1 hour')::timestamp, 'Mozilla/5.0 (Windows NT 10.0) Chrome', '10.0.0.1'),
 ('22222222-0000-4000-8000-000000000002','aaaaaaaa-0000-4000-8000-00000000000a', now()-interval '60 days', now(), (now()-interval '50 days')::timestamp, 'Mozilla/5.0 (iPhone) Safari', '10.0.0.2'),
 ('33333333-0000-4000-8000-000000000003','aaaaaaaa-0000-4000-8000-00000000000a', now()-interval '2 days', now(), null, 'Mozilla/5.0 (Android) Chrome', '10.0.0.3'),
 ('44444444-0000-4000-8000-000000000004','bbbbbbbb-0000-4000-8000-00000000000b', now()-interval '1 day', now(), (now()-interval '1 hour')::timestamp, 'Mozilla/5.0 (Macintosh) Chrome', '10.0.0.4');

set local role authenticated;
set local request.jwt.claims = '{"sub":"aaaaaaaa-0000-4000-8000-00000000000a","role":"authenticated","session_id":"11111111-0000-4000-8000-000000000001"}';
\echo -- A sees only their own active sessions: the 60-day one and the never-refreshed one are out --
select session_id, calls_enabled, is_current from public.session_devices_list();
\echo -- and the claim identifies this device --
select public.voice_calls_allowed_here() as allowed_before;
\echo -- A silences this device --
select public.session_device_set_calls('11111111-0000-4000-8000-000000000001', false) as set_to;
select public.voice_calls_allowed_here() as allowed_after;
select session_id, calls_enabled, is_current from public.session_devices_list();
\echo -- A cannot touch B's session, and the refusal does not confirm it exists --
do $p$ begin
  perform public.session_device_set_calls('44444444-0000-4000-8000-000000000004', false);
  raise notice 'UNEXPECTED: A silenced B''s device';
exception when others then raise notice 'someone else''s device refused: %', sqlerrm; end $p$;
do $p$ begin
  perform public.session_device_set_calls('99999999-0000-4000-8000-000000000099', false);
exception when others then raise notice 'a session that does not exist refused: %', sqlerrm; end $p$;
reset role;

\echo -- with no session_id claim the device still rings, and nothing is current --
set local role authenticated;
set local request.jwt.claims = '{"sub":"aaaaaaaa-0000-4000-8000-00000000000a","role":"authenticated"}';
select public.voice_calls_allowed_here() as allowed_without_claim;
select count(*) filter (where is_current) as current_rows from public.session_devices_list();
reset role;

\echo -- B sees only their own --
set local role authenticated;
set local request.jwt.claims = '{"sub":"bbbbbbbb-0000-4000-8000-00000000000b","role":"authenticated","session_id":"44444444-0000-4000-8000-000000000004"}';
select count(*) as b_sees from public.session_devices_list();
reset role;

\echo -- and the table itself is unreachable --
set local role authenticated;
set local request.jwt.claims = '{"sub":"aaaaaaaa-0000-4000-8000-00000000000a","role":"authenticated"}';
do $p$ begin
  perform count(*) from public.user_session_settings;
  raise notice 'UNEXPECTED: a client read the table directly';
exception when others then raise notice 'direct read refused: %', sqlerrm; end $p$;
reset role;
rollback;
