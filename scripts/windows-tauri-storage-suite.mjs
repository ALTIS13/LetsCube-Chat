  ...
  const scenarios = [
    ...
    },
    {
      ...
      verify: () => {
        ...
        const movedAway = arrived === ledger.beforeOrphan.size && ledger.beforeOrphan.size > 0;
        const recorded = settings?.location === ledger.orphanTarget;
        if (movedAway && !recorded) {
          ledger.defects.push(
            "a relocation whose settings write fails signs the person out and orphans their profile: " +
              ...
              "recreated empty. The data is intact but unreachable, and the session is gone.",
          );
        } else if (movedAway && recorded) {
          ledger.notes.push("settings write survived a read-only settings file (no orphan)");
        } else {
          ledger.notes.push(
            `an unwritable settings file stopped the move: ${arrived} of ${ledger.beforeOrphan.size} files reached the target, the profile stayed put`,
          );
        }
        return problems;
      },
