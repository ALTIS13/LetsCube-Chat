...
function sheetHtml(rendered) {
  ...
  return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><style>
    ...
    <p class="lead">Вариант B: у высокой картинки пропорция опускается до 0,5, высота пузыря — до 550 px.
    ...
    Здесь iPhone 430×932: пузырь шириной 310 px, у скриншота видно 82% высоты вместо 51%.
    Потолок сначала сделали 480 px, и тогда на этом телефоне выходил 71%, а «примерно 82%» получались
    только на узком 390 px — владелец посмотрел кадры и попросил поднять, теперь 82% именно здесь.
    На узких телефонах высоту держит уже не потолок, а пропорция 0,5: и при 390 px, и при 360 px — 92%.
    На компьютере — 60% вместо 53%.</p>
    ${sections}
  </body></html>`;
}
