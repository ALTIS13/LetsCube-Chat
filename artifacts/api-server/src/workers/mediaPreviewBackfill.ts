...
/**
 * The short side the new rule keeps when the source has it.
 *
 ...
 * worker's.
 */
export const PREVIEW_BACKFILL_MIN_SHORT_SIDE = 720;
 ...
 * - A preview at the cap whose short side already clears 720 is at or inside
 *   16:9, and that is precisely where the floor stops biting — the new rule
 ...
 *
 * So the candidates are the rows sitting exactly at the cap with a short side
 * under the floor. This never misses a row that needs regenerating — the sweep
 * in `tests/unit/media-preview-backfill.test.mjs` holds that — and it admits
 * one harmless false positive, a source whose long side is itself exactly 1280,
 * which regenerates to the size it already has.
 */
export function isPreviewBackfillCandidate(
  ...
  return shortSide < PREVIEW_BACKFILL_MIN_SHORT_SIDE;
}
