        ...
        <Dialog.Content className="bots-management-surface kub-glass-strong fixed bottom-0 left-0 right-0 z-[71] max-h-[92dvh] overflow-y-auto rounded-t-lg border border-[color:var(--kub-border-color)] p-5 focus:outline-none sm:bottom-auto sm:left-1/2 sm:right-auto sm:top-1/2 sm:w-[calc(100%-2rem)] sm:max-w-lg sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-lg sm:p-6">
          ...
          <form className="mt-5 space-y-4" onSubmit={submit}>
            ...
            <div>
              ...
              <textarea id="bot-create-description" value={description} onChange={(event) => { setDescription(event.target.value); setFieldErrors((current) => ({ ...current, description: undefined })); }} maxLength={512} rows={4} aria-invalid={Boolean(fieldErrors.description)} aria-describedby={fieldErrors.description ? "bot-create-description-error" : undefined} className="mt-1.5 w-full resize-none rounded-md border border-[color:var(--kub-border-color)] bg-[var(--kub-inset)] p-3 text-sm text-[color:var(--kub-text)] outline-none focus:border-[color:var(--kub-cyan)] aria-[invalid=true]:border-[color:var(--kub-danger)]" />
