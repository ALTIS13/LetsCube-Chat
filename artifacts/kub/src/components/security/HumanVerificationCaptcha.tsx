...
interface HumanVerificationCaptchaProps {
  disabled?: boolean;
...
export function HumanVerificationCaptcha({
  disabled = false,
  onTokenChange,
  resetSignal = 0,
  required = false,
  testId = "human-verification-captcha",
  ariaLabel = "Проверка защиты от автоматических запросов",
  ...
  if (!config) {
    ...
    return (
      <div
        data-testid={testId}
        className="rounded-xl border border-[color:var(--kub-warn)] bg-[color:var(--kub-warn)]/10 px-3 py-2 text-xs leading-5 text-[color:var(--kub-text)]"
      >
        Проверка защиты формы не настроена. Обратитесь в поддержку позже.
      </div>
    );
  }
